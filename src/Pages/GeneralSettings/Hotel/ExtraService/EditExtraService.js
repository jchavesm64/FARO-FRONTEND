...
import {
  ...
  UPDATE_SERVICIO,
} from "../../../../services/ServiciosExtraService";
...
const EditExtraService = ({ idBooking, updateServiceBooking }) => {
  ...
  const { data: typesService } = useQuery(OBTENER_TIPOSSERVICIOS, {
    pollInterval: 1000,
  });
  const {
    loading: loading_extraservice,
    error: error_extraservice,
    data: data_extraservice,
    startPolling,
    stopPolling,
  } = useQuery(OBTENER_SERVICIO_BY_ID, {
    variables: { id: idBooking ? idBooking : id },
    pollInterval: 1000,
  });
    ...
    try {
      ...
      if (!idBooking) {
        const { data } = await actualizar({
          variables: { id: !idBooking ? id : idBooking, input },
          errorPolicy: "all",
        });
        ...
        infoAlert(
          "Excelente",
          "Servicio actualizado para la reserva",
          "success",
          3000,
          "top-end"
        );
      ...
      infoAlert(
        "Oops",
        "Ocurrió un error inesperado al guardar el servicio",
        "error",
        3000,
        "top-end"
      );
        ...
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              ...
              breadcrumbItem="servicio"
              breadcrumbItemUrl="/hotelsettings/extraservice"
            />
            <Row>
        ...
        <Container fluid={true}>
          {!idBooking && (
            <Breadcrumbs
              title="Editar servicio"
              breadcrumbItem="Servicio"
              breadcrumbItemUrl="/hotelsettings/extraservices"
            />
          )}
          <Card className="p-4">
            <Row>
