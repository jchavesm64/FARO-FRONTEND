...
const EditPackage = ({ props, idBooking, updatePackage }) => {
  ...
  const { data: seasons } = useQuery(OBTENER_TEMPORADAS, {
    pollInterval: 1000,
  });
  const {
    loading: loading_paquete,
    error: error_paquete,
    data: data_paquete,
    startPolling,
    stopPolling,
  } = useQuery(OBTENER_PAQUETE, {
  ...
  const addTour = () => {
    if (tour) {
      ...
      if (exist) {
        infoAlert(
          "Oops",
          "Ya existe esta comodidad en la habitación",
          "warning",
          3000,
          "top-end"
        );
      ...
      infoAlert(
        "Oops",
        "No ha seleccionado una comodida",
        "error",
        3000,
        "top-end"
      );
  ...
  const onClickSave = async () => {
    try {
      ...
      if (!idBooking) {
        ...
        infoAlert(
          "Excelente",
          "Paquete actualizado para la reserva",
          "success",
          3000,
          "top-end"
        );
      ...
      infoAlert(
        "Oops",
        "Ocurrió un error inesperado al guardar el paquete",
        "error",
        3000,
        "top-end"
      );
