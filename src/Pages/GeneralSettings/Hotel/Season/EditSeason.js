...
const EditSeason = () => {
  ...
  const {
    ...
    startPolling,
    stopPolling,
  } = useQuery(OBTENER_TEMPORADA, {
    variables: { id: id },
    pollInterval: 1000,
  });
  const { data: season } = useQuery(OBTENER_TEMPORADAS, { pollInterval: 1000 });
  const { data: typeRoom } = useQuery(OBTENER_TIPOSHABITACION, {
    pollInterval: 1000,
  });
  ...
  useEffect(() => {
    if (dataSeason) {
      setTypeSeason({
        value: dataSeason.obtenerTemporadaById.tipo,
        label: dataSeason.obtenerTemporadaById.nombre,
      });
  ...
  useEffect(() => {
    setDisableSave(
      !(
        ...
        dateStart &&
        dateEnd &&
        validatePriceForPriceTypeRoom(priceTypeRoom)
      )
    );
  ...
  const seasonLimit = () => {
    ...
    return typeSeasons.filter(
      (ts) => !seasonExist.some((exist) => exist.tipo === ts.value)
    );
  };
  ...
  const onClickSave = async () => {
    try {
      ...
      infoAlert(
        "Oops",
        "Ocurrió un error inesperado al guardar la habitación",
        "error",
        3000,
        "top-end"
      );
        ...
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              ...
              breadcrumbItem="Temporada"
              breadcrumbItemUrl="/hotelsettings/season"
            />
            <Row>
                ...
                <div className="col-md-12">
                  <CardBody>
                    <DataList
                      props={{
                        handlePriceChange,
                        getFilteredTypeRoomByKey,
                        priceTypeRoom,
                      }}
                      data={
                        filterTypeRooms.length > 0 ? filterTypeRooms : typeRooms
                      }
                      type="typeroomseason"
                      displayLength={5}
                    />
