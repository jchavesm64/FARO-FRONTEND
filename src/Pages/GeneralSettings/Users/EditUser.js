...
const EditUser = (props) => {
    ...
    const { id } = useParams();
    const { loading: loading_user, error: error_user, data: data_user, startPolling, stopPolling } = useQuery(OBTENER_USUARIO, { variables: { id: id }, pollInterval: 1000 });
    ...
    useEffect(() => {
        if (data_user) {
            setUserId(data_user.obtenerUsuario.cedula)
            setUserName(data_user.obtenerUsuario.nombre)
            setCorreos(data_user.obtenerUsuario.correos || [])
            setTelefonos(data_user.obtenerUsuario.telefonos || [])
            setRoles(data_user.obtenerUsuario.roles || [])
        }
    }, [data_user])
    ...
    const onClickSave = async () => {
        try {
            ...
            console.log(input)
            const id = data_user.obtenerUsuario.id;
            ...
            <React.Fragment>
                <div className="page-content">
                    <Container fluid={true}>
                        <Breadcrumbs title="Editar usuario" breadcrumbItem="Usuarios" breadcrumbItemUrl='/users' />
                        <Row>
