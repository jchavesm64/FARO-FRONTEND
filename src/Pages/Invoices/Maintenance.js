...
const InvoiceMaintenance = ({ ...props }) => {
  ...
  const { data: dataCurrencyTypes } = useQuery(
  ...
  const { data: dataSaleConditions } = useQuery(
    OBTENER_FACTURAS_PARAMETROS_BY_TYPE,
    { variables: { type: "saleConditions" } }
  );

  const { data: dataDocumentTypes } = useQuery(
    OBTENER_FACTURAS_PARAMETROS_BY_TYPE,
    { variables: { type: "documentTypes" } }
  );

  const { data: dataMateriasPrimas } = useQuery(
  ...
  useEffect(() => {
    if (dataCurrencyTypes) {
      const options = dataCurrencyTypes.obtenerFacturasParametrosByType.map(
        (item) => ({
          value: item.id,
          label: item.value,
        })
    ...
    if (dataPaymentMethods) {
      const options = dataPaymentMethods.obtenerFacturasParametrosByType.map(
        (item) => ({
          value: item.id,
          label: item.value,
        })
    ...
    if (dataSaleConditions) {
      const options = dataSaleConditions.obtenerFacturasParametrosByType.map(
        (item) => ({
          value: item.id,
          label: item.value,
        })
    ...
    if (dataDocumentTypes) {
      const options = dataDocumentTypes.obtenerFacturasParametrosByType.map(
        (item) => ({
          value: item.id,
          label: item.value,
        })
  ...
  const getDataMeteriasPrimas = () => {
    if (dataMateriasPrimas) {
      if (dataMateriasPrimas.obtenerTodasMateriasPrimas) {
        return dataMateriasPrimas.obtenerTodasMateriasPrimas.filter(
          (value, index) => {
      ...
      prevArticulosLista.find((value, index) => {
        if (value.codigoCabys === articulo.codigoCabys) {
          value.cantidadArticulo =
            parseInt(value.cantidadArticulo) + parseInt(cantidad.value);
          findVali = true;
          return value;
        }
  ...
  useEffect(() => {
    if (props.data) {
      if (
        ...
        dataPaymentMethods &&
        dataCurrencyTypes &&
        clienteFacturar === false &&
        selectedTipoFactura === null &&
        selectedCondicionVenta === null &&
        ...
        if (props.data.selectedTipoFactura) {
          dataDocumentTypes.obtenerFacturasParametrosByType.map(
            (value, index) => {
        ...
        if (props.data.selectedCondicionVenta) {
          dataSaleConditions.obtenerFacturasParametrosByType.map(
            (value, index) => {
        ...
        if (props.data.selectedMetodoPago) {
          dataPaymentMethods.obtenerFacturasParametrosByType.map(
            (value, index) => {
        ...
        if (props.data.selectedTipoMoneda) {
          dataCurrencyTypes.obtenerFacturasParametrosByType.map(
            (value, index) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (
          ...
          myHeaders.append(
            "Cookie",
            "session=qifO9RMl_v6U89RqK1yBNeYrK40-gYum763TWWpGmus"
          );
                  ...
                  <>
                    ...
                    <div className="col-md-2 mb-3">
                      <button
                        ...
                        disabled={clienteCedula.length > 0 ? false : true}
                      >
              ...
              <Row>
                ...
                <div className="col-md-6 mb-3">
                  ...
                  <Select
                    ...
                    menuPosition="fixed"
                    isDisabled={
                      props.data && props.data.selectedTipoFactura
                        ? true
                        : false
                    }
                  />
                      ...
                      <>
                        {articulosLista.map((asset, i) => (
                          <tr key={`asset-${i}`}>
                            ...
                            <td>
                              {asset.cantidadArticulo * asset.precioCompra}
                            </td>
                            <td>%13</td>
                            <td>
                              <ButtonIconTable
