...
const Booking = () => {
  ...
  const { data: data_booking } = useQuery(OBTENER_RESERVAS, {
    pollInterval: 1000,
  });
  ...
  const filterBookings = () => {
    const filtered = booking.filter((b) => {
      ...
      if (filterCriteria.clienteNombre) {
        ...
        if (
          !clienteNombreLower.includes(filterLower) &&
          !clienteCodigoLower.includes(filterLower)
        ) {
          matches = false;
        }
      }
      ...
      if (
        filterCriteria.fechaReserva &&
        new Date(Number(b.fechaReserva)).toLocaleDateString() !==
          new Date(filterCriteria.fechaReserva).toLocaleDateString()
      ) {
        matches = false;
      }
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumbs
            title="Reservas"
            breadcrumbItem="Nueva Reserva"
            breadcrumbItemUrl="/reception/availability"
          />
        </Container>
              ...
              <div className="col-md-12">
                <Card>
                  <CardBody>
                      ...
                      <Col>
                        <FormGroup>
                          ...
                          <Input
                            ...
                            value={filterCriteria.clienteNombre}
                        ...
                        <Col md={4}>
                          <FormGroup>
                            ...
                            <Input
                              ...
                              value={filterCriteria.fechaReserva}
                              onChange={handleFilterChange}
                            />
                          </FormGroup>
                        </Col>
                    ...
                    <Button color="primary" onClick={filterBookings}>
                      Filtrar
                    </Button>
                  </CardBody>
                </Card>
                ...
                {filteredBooking.map((b) => (
                  <Link
                    to={
                      ...
                      b.estado === "Incompleto" ||
                      b.estado === "Conflicto"
                        ? `/reception/availability/editbooking/${b.id}`
                        : "#"
                    }
                    style={{ textDecoration: "none" }}
                    className="card_home_link p-0 m-0"
                    key={b.id}
                  >
                    <Card className="card_booking p-0 mb-2 overflow-hidden">
                      <CardHeader
                        className={`d-flex justify-content-between text-primary-foreground bg-${
                          estadoColores[b.estado] || "bg-primary"
                        }`}
                      >
                        <CardTitle className="text-lg">
                          <span className="fs-5 m-0 ms-1 mb-2 span_color">
                            Reservación:
                          </span>{" "}
                          {b.id.slice(0, 10)}...
                        </CardTitle>
                      ...
                      <CardBody className="pt-2">
                        ...
                        <div className="d-flex justify-content-around">
                          <Badge
                            color={`${estadoColores[b.estado] || "bg-primary"}`}
                            className="mr-2 fs-6"
                            variant={
                              b.estado === "Pendiente" ? "outline" : "default"
                            }
                          >
                            {b.estado}
                          </Badge>
                          ...
                          >
                            {
                              typesBooking.find((item) => item.value === b.tipo)
                                .label
                            }
                          </Badge>
                        </div>
