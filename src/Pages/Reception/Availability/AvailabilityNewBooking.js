...
const ReceptionHome = () => {
  ...
  const { data: data_rooms } = useQuery(OBTENER_HABITACIONES, {
    pollInterval: 1000,
  });
  const { data: tiposHabitacion } = useQuery(OBTENER_TIPOSHABITACION, {
    pollInterval: 1000,
  });
  const { data: data_reservaHabitacion } = useQuery(
    OBTENER_RESERVAHABITACIONES,
    { pollInterval: 1000 }
  );
    ...
    const getData = () => {
      if (data_reservaHabitacion) {
        if (data_reservaHabitacion?.obtenerReservaHabitaciones) {
          return data_reservaHabitacion?.obtenerReservaHabitaciones.filter(
            (value) => {
              return value;
            }
          );
  ...
  const calcularDisponibilidadPorDia = (mes, anio) => {
    ...
    for (let dia = 1; dia <= diasEnMes; dia++) {
      ...
      const habitacionesDisponibles =
        totalHabitaciones - habitacionesReservadas;

      const porcentajeDisponibilidad =
        (100 * habitacionesReservadas) / totalHabitaciones;
    ...
    const getTiposHabitacion = () => {
      if (tiposHabitacion) {
        if (tiposHabitacion.obtenerTiposHabitaciones) {
          return tiposHabitacion.obtenerTiposHabitaciones.filter(
            (value, index) => {
              return value;
            }
          );
    ...
    const typeRoomAvailability = () => {
      ...
      const isRoomReserved = (habitacionId, month, year) => {
        return reservaHabitacion.some((reserva) => {
          ...
          // Verificamos si la reserva está en el mismo mes y año
          const overlaps =
            (entradaAño === year && entradaMes === month) ||
            (salidaAño === year && salidaMes === month) ||
            (entradaAño < year && salidaAño >= year);
      ...
      rooms?.forEach((habitacion) => {
        ...
        const roomType = availability.find(
          (room) => room.nombre === tipoNombre
        );
        ...
        if (roomType) {
          ...
          roomType.porcentajeDisponible =
            roomType.cantidadTotal > 0
              ? (
                  (roomType.cantidadDisponible / roomType.cantidadTotal) *
                  100
                ).toFixed(0)
              : 0;
        }
  ...
  function filtrarReservasPorFecha(reservas, fechaSeleccionada) {
    ...
    return reservas?.filter((reserva) => {
      ...
      return (
        fechaSeleccionadaMs >= fechaEntradaMs &&
        fechaSeleccionadaMs <= fechaSalidaMs
      );
    });
  }
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumbs
            title="Disponibilidad y nuevas reservas"
          ...
          <Card key="availability" className="col-md-12 p-2">
            <Row
              key="rowavailability"
              className="d-flex justify-content-between m-2 border-bottom align-items-center shadow_calendar_button"
            >
              <div key="maindiv" className="col-md-2 col-sm-12 mb-3">
                <Link
                  key="linkviewbooking"
                  to="/reception/availability/booking"
                >
                  <button
              ...
              <div className="col-md-2 col-sm-12 mb-3">
                <Link
                  key="linknewbooking"
                  to="/reception/availability/newbooking"
                >
                  <button
            ...
            <Row key="calendarmain" className="mt-3">
              <div
                key="calendardiv"
                className="col-md-5 m-2 mt-0 shadow_calendar rounded"
              >
                <Col
                  key="calendarselect"
                          ...
                          <Button
                            ...
                            style={{
                              backgroundColor: getColorByPercentage(
                                parseInt(dia.porcentajeDisponibilidad)
                              ),
                            }}
                            onClick={() => getData(dia, true)}
                          >
                            <span
                              key={`span${dia.dia}${week}`}
                              className="fs-5"
                              style={{
                                color:
                                  parseInt(dia.porcentajeDisponibilidad) === 100
                                    ? "white"
                                    : "black",
                              }}
                            >
              ...
              <div key="listtyperoom" className="col-md-6">
                <Card>
                  <CardBody>
                    <DataList
                      data={
                        roomAvailability !== undefined ? roomAvailability : []
                      }
                      type="typeroomdata"
                      displayLength={3}
                    />
            ...
            <Modal key="modalavailability" isOpen={modal} toggle={toggle}>
              <ModalHeader key="modalheader" toggle={toggle}>
                <span className="fs-4 m-0 span_package_color">
                  Información de reserva por día
                </span>
              </ModalHeader>
              <ModalBody key="modalbody">
                ...
                >
                  <label
                    key="labelmodalmain"
                    className="fs-6 m-0 ms-4 span_package_color"
                  >
                    <strong
                      key="modaliconday"
                      className="mdi mdi-calendar-range me-1"
                    />
                    <strong>Detalles del día: </strong>{" "}
                    <span className="fs-6 label_package_color">{`${
                      dataModal.day
                    }/${month + 1}/${year}`}</span>
                    <strong> &nbsp; </strong>
                    <strong>Habitaciones disponibles: </strong>{" "}
                    <span className="fs-6 label_package_color">{`${dataModal.rooms}`}</span>
                  </label>
                  <label className="fs-6 m-0 ms-4 span_package_color">
                    ...
                    <span className="fs-6 label_package_color">{`${dataModal.percent}%`}</span>
                  </label>
                  <label className="fs-6 m-0 ms-4 span_package_color">
                    <strong className="mdi mdi-check-circle-outline me-1" />
                    <strong>Estado: </strong>
                    <span className="fs-6 label_package_color">
                      {dataModal.percent <= 25
                        ? "Baja ocupación"
                        : dataModal.percent <= 50
                        ? "Ocupación media"
                        : dataModal.percent <= 80
                        ? "Alta ocupación"
                        : "Ocupación completa"}
                    </span>
                  </label>
                    ...
                    <label className="fs-6 m-0 ms-4 span_package_color">
                      ...
                      <div className="ms-5">
                        {filtrarReservasPorFecha(
                          reservaHabitacion,
                          `${year}-${month + 1}-${dataModal.day}`
                        )?.map((data) => (
                          <div key={data.habitacion.numeroHabitacion}>
                            Habitación:{" "}
                            <span className="fs-6 label_package_color">{`${data.habitacion.numeroHabitacion}`}</span>
                          </div>
                        ))}
                      </div>
                    </label>
