...
const EditSupplier = ({ props, supplier }) => {
    ...
    useEffect(() => {
        setName(supplier.empresa)
        setSupplierId(supplier.cedula)
        setCountry(getPais(supplier.pais))
        setCountryState(getCiudad(supplier.ciudad))
        setAddress(supplier.direccion)
        setTelefonos(supplier.telefonos)
        setCorreos(supplier.correos)
        setRedes(supplier.redes)
        setCity(supplier.city)
        setStreet(supplier.calle)
        setPostalCode(supplier.cp)
        setProvedurias(supplier.provedurias && supplier.provedurias.length > 0 ? supplier.provedurias.map(p => ({ value: p.id, label: p.tipo })) : [])
        setVencimientoPago(supplier.vencimientoPago)
        setVencimientoPagoTipo(supplier.vencimientoPagoTipo !== null ? { value: supplier.vencimientoPagoTipo, label: supplier.vencimientoPagoTipo } : null)
        setAlertaDiasAntes(supplier.alertaDiasAntes)
    }, [supplier])
    ...
    const onClickSave = async () => {
        try {
            ...
            const { data } = await actualizar({ variables: { id: supplier.id, input }, errorPolicy: 'all' });
