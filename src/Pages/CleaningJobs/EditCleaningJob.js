...
const EditCleaningJob = ({ props, cleaningJob }) => {
    ...
    useEffect(() => {
        setNombre(cleaningJob.nombre)
        setCodigo(cleaningJob.codigo)
        setUbicacion(cleaningJob.ubicacion ? { label: cleaningJob.ubicacion.nombre, value: cleaningJob.ubicacion.id } : null)
        setLineas(cleaningJob.areas)
    }, [cleaningJob])
    ...
    const onSave = async () => {
        try {
            ...
            const { data } = await actualizar({ variables: { id: cleaningJob.id, input }, errorPolicy: 'all' });
            ...
            console.log(error);
            infoAlert('Oops', 'Ocurrió un error inesperado al actualizar el puesto de limpieza', 'error', 3000, 'top-end')
            setDisableSave(false)
        }
    }
...
export default EditCleaningJob;
