...
import {
  convertDataPurchaseOrdersExcel,
...
const PurchaseOrders = ({ ...props }) => {
  ...
  const {
    loading: load_ordenes_compra,
    data: data_ordenes_compra,
    refetch,
  } = useQuery(OBTENER_ORDENES_COMPRA, { pollInterval: 1000 });
  const [desactivar] = useMutation(DELETE_ORDEN_COMPRA, {
    update(cache, { data: { desactivarOrdenCompra } }) {
      const { obtenerOrdenesCompra } = cache.readQuery({
        query: OBTENER_ORDENES_COMPRA,
      });
      cache.writeQuery({
        query: OBTENER_ORDENES_COMPRA,
        data: {
          obtenerOrdenesCompra: obtenerOrdenesCompra.filter(
            (orden) => orden.id !== desactivarOrdenCompra.id
          ),
        },
      });
    },
  ...
  const getData = () => {
    if (data_ordenes_compra) {
      if (data_ordenes_compra.obtenerOrdenesCompra) {
        return data_ordenes_compra.obtenerOrdenesCompra.filter(
          (value, index) => {
            if (filter !== "") {
  ...
  const onClickExportExcel = () => {
    exportAndDownloadExcel(
      "ordenesCompra",
      convertDataPurchaseOrdersExcel(data)
    );
  };
        ...
        <Container fluid={true}>
          ...
          <Row className="flex" style={{ alignItems: "flex-end" }}>
            <div className="col-md-10 mb-3">
              <label htmlFor="search-input" className="col-md-2 col-form-label">
